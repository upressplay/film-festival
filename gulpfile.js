...
function jsMinify() {
	return gulp
	.src(['src/js/lib/**/*.js','src/js/**/*.js']) // path to your files, put all code modules in lib to avoid undefined errors
...
function css(){
	...
	.pipe(autoprefixer({ //add vendor prefixes for browsers in our support table
		overrideBrowserslist: ["Chrome >= 32", "ie >=9", "Firefox >=27", "Safari >=7", "last 2 Opera versions", "iOS >= 6", "Android >= 4"],
		cascade: false
	}))
	.pipe(plumber({errorHandler: onError})) //if the minifier crashes, fail gracefully
...
function images(){ //optimize all image files
	return gulp
...
function htmlHint() { //validate html files
	return gulp
	.src('src/**/*.html')
	.pipe(plumber({errorHandler: onError})) //if the validator crashes, fail gracefully
	.pipe(htmlhint())
...
function copyPages() { //copy all html and php files over
	return gulp
	.src('src/**/*.+(html|php)')
